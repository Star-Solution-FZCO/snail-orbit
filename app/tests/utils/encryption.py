...
def encrypt_with_aes(text: str) -> tuple[bytes, str]:
    ...
    cipher = Cipher(algorithms.AES(aes_key), modes.ECB())  # nosec: B305
...
def encrypt_aes_key_with_x25519(
    aes_key: bytes, recipient_public_key_bytes: bytes
) -> dict[str, str]:
...
def decrypt_aes_key_with_x25519(
    ...
    decryptor = cipher.decryptor()
    aes_key = decryptor.update(encrypted_key) + decryptor.finalize()
